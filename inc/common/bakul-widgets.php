...
function bakul_widgets_init() {
     ...
     */
    register_sidebar( [
        'name'          => esc_html__( 'Blog Sidebar', 'bakul' ),
        'id'            => 'blog-sidebar',
     ...
     */
    register_sidebar( [
        'name'          => esc_html__( 'Product Sidebar', 'bakul' ),
        'id'            => 'product-sidebar',
    ...
    register_sidebar( [
        'name'          => esc_html__( 'Services Sidebar', 'bakul' ),
    ...
    for ( $num = 1; $num <= $footer_widgets; $num++ ) {
        register_sidebar( [
            'name'          => sprintf( esc_html__( 'Footer %1$s', 'bakul' ), $num ),
            'id'            => 'footer-' . $num,
            'description'   => sprintf( esc_html__( 'Footer Column %1$s', 'bakul' ), $num ),
            'before_widget' => '<div id="%1$s" class="footer__img mb-50 footer-cols-'.$num.' %2$s">',
            'after_widget'  => '</div>',
    ...
    if ( $footer_style_2_switch ) {
        for ( $num = 1; $num <= $footer_widgets; $num++ ) {
            ...
            register_sidebar( [
                'name'          => sprintf( esc_html__( 'Footer Style 2 : %1$s', 'bakul' ), $num ),
                'id'            => 'footer-2-' . $num,
                'description'   => sprintf( esc_html__( 'Footer Style 2 : %1$s', 'bakul' ), $num ),
    ...
    if ( $footer_style_3_switch ) {
        for ( $num = 1; $num <= $footer_widgets; $num++ ) {
            ...
            register_sidebar( [
                'name'          => sprintf( esc_html__( 'Footer Style 3 : %1$s', 'bakul' ), $num ),
                'id'            => 'footer-3-' . $num,
                'description'   => sprintf( esc_html__( 'Footer Style 3 : %1$s', 'bakul' ), $num ),
...
add_action( 'widgets_init', 'bakul_widgets_init' );